#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""マンション管理士 試験ガイド向けの本文・FAQ 生成（量産テンプレ差し替え用）。"""
...
def topic_from_row(row: dict[str, str]) -> str:
    ...
    if len(title) >= 4 and not title.startswith("マ管"):
        return title
    return slug.replace("-", " ")
...
def _heading_制度改定(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"マ管業法・民法・都市計画法・建築基準法など、"
        f"{topic}の背景法令は改正やガイドライン更新があります。"
        f"試験は制度の「考え方」とマ管受験者の対応原則が中心ですが、数値要件は公式テキストの版を確認してください。",
...
def _heading_分野位置づけ(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    ...
    return two_paragraphs(
        ...
        f"マ管受験者試験では、現場の判断場面（気づき・相談・配慮・環境改善・専門職連携）として問われます。",
...
def _heading_基礎知識(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    field = field_name_from_slug(slug) or topic
    return two_paragraphs(
        f"{field}の基礎として、キーワードの定義（重要事項説明、35条書面、媒介契約、マ管業法の免許、都市計画法など）を"
        f"用語解説で確認してください。試験では長い定義文の一部が空欄になったり、似た語句と並べ替えられたりします。",
        f"暗記カードを作る場合は「誰が・いつ・何をするか」の3点セットでまとめると、"
        f"事例問題の主体（マ管受験者／産業医／人事）を見分けやすくなります。{_practice_note(topic)}",
    )
...
def _heading_頻出論点(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    field = field_name_from_slug(slug) or topic
    return two_paragraphs(
        f"{field}では、マ管受験者の具体的行動（早期発見、傾聴、記録、専門職へのつなぎ、職場環境へのフィードバック）が"
...
def _heading_他分野関連(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    field = field_name_from_slug(slug) or topic
    return two_paragraphs(
        f"「{field}」は、民法・借地借家法（民法）やマ管業法（35条書面）など、"
...
def _heading_受験資格(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{EXAM}（マ管）の受験資格は、{OFFICIAL}の受験要項で確認します。"
        f"マ管受験者としての位置づけや、実務経験・研修の要件など、自分に該当するかをチェックリスト化してください。",
...
def _heading_過去問年度別(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"解説を読む際は、マ管受験者の行動として正しいか／法令上の主体が誰か、を必ず確認してください。"
...
def _heading_間違い分類(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"誤答の理由を「知識不足」「用語混同」「読み飛ばし」「主体の取り違え（マ管受験者 vs 産業医）」"
...
def _heading_マ管受験者役割(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"マ管受験者は、{topic}の場面で「早期発見・傾聴・安全配慮・記録・専門職連携・職場環境改善」"
        f"のバランスを取る主体として問われます。医学的診断や治療判断は医師の領域であり、マ管受験者単独では行いません。",
...
def _heading_プライバシー(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"マ管不調に関する情報は個人情報・要配慮個人情報として慎重に扱います。"
        f"マ管受験者は業務上必要な範囲で情報を共有し、病名の詳細を不必要に広げない対応が求められます。",
...
def _heading_フォローアップ(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"「復職したら終わり」ではなく、再発予防がマ管受験者の役割として問われます。",
...
def _heading_相談窓口(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"マ管受験者は相談を受けた場合の初動（傾聴、記録、担当者への連携）を理解しておく必要があります。",
...
def _heading_再発防止(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"再発防止には、ハラスメント防止方針の周知、相談窓口の明示、マ管受験者向け研修、"
...
def _heading_5ステップ(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"マ管受験者は③④⑤で中心役割を担いますが、医学的判断は主治医・産業医の領域です。"
...
def _heading_試験目的(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{EXAM}（マ管）は、マ管受験者が職場でマ管対策を実践するための"
...
HEADING_MAP: dict[str, Callable[..., str]] = {
    ...
    "合格後の手続き": lambda t, s, g, c: two_paragraphs(
        ...
        f"{t}に合格した場合も、マ管受験者としての実務研修や社内規程の更新が別途必要なことがあります。"
    ...
    "マ管受験者の役割": _heading_マ管受験者役割,
    "マ管受験者の関与": _heading_マ管受験者役割,
...
def faq_answer_for(question: str, topic: str, slug: str, row: dict[str, str], faq_index: int = 1) -> str:
    ...
    # FAQ 役割分担で類似回答を避ける
    if faq_index == 1:
        if "マ管受験者" in q and ("診断" in q or "判断" in q):
            text = (
                "マ管受験者は医学的診断を行いません。不調の気づき、傾聴、安全配慮、"
        ...
        if "復職" in q or "復帰" in q:
            ...
            text = (
                f"試験では「{topic}」が、マ管受験者の行動として正しいか／主体を取り違えていないか、"
...
def lead_for(row: dict[str, str], topic: str) -> str:
    ...
    base = (
        f"{EXAM}の{topic}について、マ管受験者が現場で迷いやすい論点と"
...
def meta_description_for(row: dict[str, str], topic: str) -> str:
    ...
    if genre == "用語ハブ活用法":
        return (
            ...
            f"用語解説・演習・比較表との学習順と、マ管受験者が押さえる論点を解説します。"
        )[:165]
    if genre == "試験概要":
        return (
    ...
    return (
        f"{EXAM}の{topic}について、マ管受験者試験で問われる論点と学習の進め方を解説。"
