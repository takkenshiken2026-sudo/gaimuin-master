#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""管理業務主任者 試験ガイド向けの本文・FAQ 生成（量産テンプレ差し替え用）。"""
...
def topic_from_row(row: dict[str, str]) -> str:
    ...
    if len(title) >= 4 and not title.startswith("管業"):
        return title
...
def _heading_よくある誤解(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{topic}では、「管業受験者が診断する」「休職勧奨＝解雇」"
        f"「ストレスチェック結果を個別に上司が全部見る」など、現場イメージと制度のズレが誤答の温床になります。"
        f"特に管理組合と理事会、管理委託と賃貸借の混同は四択で頻出です。",
...
def _heading_独学前に公式情報を確認(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"独学を始める前に、{OFFICIAL}で受験資格・試験日程・出題範囲（10分野）・合格基準をメモしてください。"
...
def _heading_教材参考書(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{topic}向けには、{ORG}発行の公式テキスト（管業）を軸に置くのが基本です。"
        f"問題集は年度・版の表記を確認し、10分野すべてをカバーしているかを目次でチェックしてください。",
        f"教材を増やしすぎるより、1冊を2周してから演習問題・一問一答で穴を見つける方が、"
        f"管業受験者の実務イメージもつきやすくなります。{_practice_note(topic)}",
    )


def _heading_過去問現在地(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"最初の過去問は、{topic}に限定せず1回分を通しで解き、10分野のうちどこで迷ったかを記録します。"
        f"たとえば「民法・借地借家法」と「法令」の問題を取り違えた、など分野名でメモすると復習が楽です。",
...
def _heading_直前期絞り込み(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"10分野のうち得点率が低い領域を1つ選び、用語10語＋演習20問程度をセットで見直すのが目安です。",
        f"前日は暗記の詰め込みより、睡眠と当日の持ち物・会場確認を優先してください。"
        f"数値（修繕積立金、管理費、総会決議、管理委託契約、登録など）は早見表タブで最終確認すると安心です。",
    )
...
def _heading_制度改定(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"管業業法・民法・都市計画法・建築基準法など、"
        f"{topic}の背景法令は改正やガイドライン更新があります。"
        f"試験は制度の「考え方」と管業受験者の対応原則が中心ですが、数値要件は公式テキストの版を確認してください。",
...
def _heading_分野位置づけ(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    ...
    return two_paragraphs(
        f"公式テキストの10分野のうち、「{field}」は{topic}の理解に直結する部分です。"
        f"管業受験者試験では、現場の判断場面（気づき・相談・配慮・環境改善・専門職連携）として問われます。",
        f"他領域とのつながりを意識すると、事例問題の選択肢が読みやすくなります。"
        f"たとえば民法・借地借家法の領域は、個別配慮や職場環境評価とセットで出題されることが多いです。",
    )
...
def _heading_基礎知識(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    field = field_name_from_slug(slug) or topic
    return two_paragraphs(
        f"{field}の基礎として、キーワードの定義（重要事項説明、35条書面、媒介契約、管業業法の免許、都市計画法など）を"
        f"用語解説で確認してください。試験では長い定義文の一部が空欄になったり、似た語句と並べ替えられたりします。",
        f"暗記カードを作る場合は「誰が・いつ・何をするか」の3点セットでまとめると、"
        f"事例問題の主体（管業受験者／産業医／人事）を見分けやすくなります。{_practice_note(topic)}",
    )
...
def _heading_頻出論点(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    field = field_name_from_slug(slug) or topic
    return two_paragraphs(
        f"{field}では、管業受験者の具体的行動（早期発見、傾聴、記録、専門職へのつなぎ、職場環境へのフィードバック）が"
...
def _heading_他分野関連(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    field = field_name_from_slug(slug) or topic
    return two_paragraphs(
        f"「{field}」は、民法・借地借家法（民法）や管業業法（35条書面）など、"
        f"他領域のキーワードとセットで事例が構成されることがあります。"
        f"関連用語リンクから隣接する用語を2〜3件読むと、長文問題の文脈がつかみやすくなります。",
        f"10分野をバランスよく学ぶため、弱点分野ばかりでなく、得意分野も月1回は演習で維持してください。"
...
def _heading_受験資格(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{EXAM}（管業）の受験資格は、{OFFICIAL}の受験要項で確認します。"
        f"管業受験者としての位置づけや、実務経験・研修の要件など、自分に該当するかをチェックリスト化してください。",
...
def _heading_申込前チェック(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"公式テキストの版・10分野の学習進捗 ⑥演習問題で弱点分野の把握。"
...
def _heading_過去問年度別(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"過去問はまず1つの実施回を通しで解き、10分野ごとの正答率をざっくり把握します。"
        f"{topic}に関連する分野の問題数が多い回を選ぶより、全体の出題バランスを見る方が初期分析には有効です。",
        f"解説を読む際は、管業受験者の行動として正しいか／法令上の主体が誰か、を必ず確認してください。"
...
def _heading_模試一問(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"一問一答は、通勤時間などの隙間に10分野を短時間で確認するのに向いています。"
...
def _heading_間違い分類(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"誤答の理由を「知識不足」「用語混同」「読み飛ばし」「主体の取り違え（管業受験者 vs 産業医）」"
        f"に分類すると、次の対策が決まります。混同が多い場合は比較表タブを優先してください。",
        f"ノートの1行例：「第○問／民法・借地借家法／『記録の開示範囲』を混同」。"
...
def _heading_管業受験者役割(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"管業受験者は、{topic}の場面で「早期発見・傾聴・安全配慮・記録・専門職連携・職場環境改善」"
        f"のバランスを取る主体として問われます。医学的診断や治療判断は医師の領域であり、管業受験者単独では行いません。",
...
def _heading_プライバシー(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"管業不調に関する情報は個人情報・要配慮個人情報として慎重に扱います。"
        f"管業受験者は業務上必要な範囲で情報を共有し、病名の詳細を不必要に広げない対応が求められます。",
...
def _heading_フォローアップ(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"「復職したら終わり」ではなく、再発予防が管業受験者の役割として問われます。",
...
def _heading_相談窓口(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"管業受験者は相談を受けた場合の初動（傾聴、記録、担当者への連携）を理解しておく必要があります。",
...
def _heading_再発防止(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"再発防止には、ハラスメント防止方針の周知、相談窓口の明示、管業受験者向け研修、"
...
def _heading_5ステップ(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"管業受験者は③④⑤で中心役割を担いますが、医学的判断は主治医・産業医の領域です。"
...
def _heading_試験目的(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{EXAM}（管業）は、管業受験者が職場で管業対策を実践するための"
        f"知識・態度を評価する試験です。部下の変化への気づき、安全配慮、民法・借地借家法、環境改善が中心テーマです。",
        f"10分野は公式テキストに沿って構成されています。"
...
def _heading_10分野(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    items = "\n".join(f"- {f}" for f in SEVEN_FIELDS)
    return two_paragraphs(
        f"出題範囲は公式テキストに沿った次の10分野です。\n{items}",
...
HEADING_MAP: dict[str, Callable[..., str]] = {
    ...
    "管業受験者の役割": _heading_管業受験者役割,
    "管業受験者の関与": _heading_管業受験者役割,
    ...
    "試験の目的と管業の位置づけ": _heading_試験目的,
    "試験形式と合格基準（公式で再確認）": _heading_試験形式,
    "出題範囲10分野の全体像": _heading_10分野,
...
def _keyword_fallback(heading: str, topic: str, slug: str, genre: str) -> str:
    ...
    checks: list[tuple[tuple[str, ...], Callable[..., str]]] = [
        ...
        (("合格", "難易度", "合格率"), lambda t, s, g, c: two_paragraphs(
            ...
            f"{topic}の学習時間は、10分野のバランスと仕事の繁忙期を考慮して設定します。",
        )),
...
def faq_answer_for(question: str, topic: str, slug: str, row: dict[str, str], faq_index: int = 1) -> str:
    ...
    # FAQ 役割分担で類似回答を避ける
    if faq_index == 1:
        if "管業受験者" in q and ("診断" in q or "判断" in q):
            text = (
                "管業受験者は医学的診断を行いません。不調の気づき、傾聴、安全配慮、"
        ...
        if "復職" in q or "復帰" in q:
            ...
            text = (
                f"試験では「{topic}」が、管業受験者の行動として正しいか／主体を取り違えていないか、"
...
def lead_for(row: dict[str, str], topic: str) -> str:
    ...
    base = (
        f"{EXAM}の{topic}について、管業受験者が現場で迷いやすい論点と"
        f"試験での出題パターンを整理する記事です。"
    )
    if genre == "用語ハブ活用法":
        base += f"用語解説で定義を確認したあと、演習と比較表で理解を深める流れを示します。"
    elif genre == "試験概要":
        base += "10分野の全体像と公式情報の確認方法から学習を始めたい人向けです。"
    else:
...
def meta_description_for(row: dict[str, str], topic: str) -> str:
    ...
    if genre == "用語ハブ活用法":
        return (
            ...
            f"用語解説・演習・比較表との学習順と、管業受験者が押さえる論点を解説します。"
        )[:165]
    if genre == "試験概要":
        return (
            f"{EXAM}の概要・10分野・合格基準の確認方法。"
    ...
    return (
        f"{EXAM}の{topic}について、管業受験者試験で問われる論点と学習の進め方を解説。"
