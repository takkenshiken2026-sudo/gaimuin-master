#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""賃貸不動産経営管理士 試験ガイド向けの本文・FAQ 生成（量産テンプレ差し替え用）。"""
...
OFFICIAL = "賃貸不動産経営管理士試験センター（公式）"
ORG = "賃貸不動産経営管理士試験センター"
SEVEN_FIELDS = (
    "賃管法令・制度",
    "契約・実務",
    "設備・税務・その他",
...
def topic_from_row(row: dict[str, str]) -> str:
    ...
    if len(title) >= 4 and not title.startswith("賃管"):
        return title
    return slug.replace("-", " ")
...
def _heading_よくある誤解(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{topic}では、「賃管受験者が診断する」「休職勧奨＝解雇」"
        f"「ストレスチェック結果を個別に上司が全部見る」など、現場イメージと制度のズレが誤答の温床になります。"
        f"特に敷金返還と原状回復、更新と再契約の混同は四択で頻出です。",
...
def _heading_過去問現在地(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"たとえば「賃管法令・制度」と「法令」の問題を取り違えた、など分野名でメモすると復習が楽です。",
...
def _heading_直前期絞り込み(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"数値（敷金、礼金、更新料、原状回復、建物設備など）は早見表タブで最終確認すると安心です。",
    )
...
def _heading_制度改定(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"賃管業法・民法・都市計画法・建築基準法など、"
        f"{topic}の背景法令は改正やガイドライン更新があります。"
        f"試験は制度の「考え方」と賃管受験者の対応原則が中心ですが、数値要件は公式テキストの版を確認してください。",
...
def _heading_分野位置づけ(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    ...
    return two_paragraphs(
        ...
        f"賃管受験者試験では、現場の判断場面（気づき・相談・配慮・環境改善・専門職連携）として問われます。",
        f"他領域とのつながりを意識すると、事例問題の選択肢が読みやすくなります。"
        f"たとえば賃管法令・制度の領域は、個別配慮や職場環境評価とセットで出題されることが多いです。",
    )
...
def _heading_基礎知識(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    field = field_name_from_slug(slug) or topic
    return two_paragraphs(
        f"{field}の基礎として、キーワードの定義（重要事項説明、35条書面、媒介契約、賃管業法の免許、都市計画法など）を"
        f"用語解説で確認してください。試験では長い定義文の一部が空欄になったり、似た語句と並べ替えられたりします。",
        f"暗記カードを作る場合は「誰が・いつ・何をするか」の3点セットでまとめると、"
        f"事例問題の主体（賃管受験者／産業医／人事）を見分けやすくなります。{_practice_note(topic)}",
    )
...
def _heading_頻出論点(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    field = field_name_from_slug(slug) or topic
    return two_paragraphs(
        f"{field}では、賃管受験者の具体的行動（早期発見、傾聴、記録、専門職へのつなぎ、職場環境へのフィードバック）が"
...
def _heading_他分野関連(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    field = field_name_from_slug(slug) or topic
    return two_paragraphs(
        f"「{field}」は、賃管法令・制度（民法）や賃管業法（35条書面）など、"
...
def _heading_受験資格(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{EXAM}（賃管）の受験資格は、{OFFICIAL}の受験要項で確認します。"
        f"賃管受験者としての位置づけや、実務経験・研修の要件など、自分に該当するかをチェックリスト化してください。",
...
def _heading_過去問年度別(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"解説を読む際は、賃管受験者の行動として正しいか／法令上の主体が誰か、を必ず確認してください。"
...
def _heading_間違い分類(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"誤答の理由を「知識不足」「用語混同」「読み飛ばし」「主体の取り違え（賃管受験者 vs 産業医）」"
        f"に分類すると、次の対策が決まります。混同が多い場合は比較表タブを優先してください。",
        f"ノートの1行例：「第○問／賃管法令・制度／『記録の開示範囲』を混同」。"
...
def _heading_賃管受験者役割(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"賃管受験者は、{topic}の場面で「早期発見・傾聴・安全配慮・記録・専門職連携・職場環境改善」"
        f"のバランスを取る主体として問われます。医学的診断や治療判断は医師の領域であり、賃管受験者単独では行いません。",
...
def _heading_プライバシー(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"賃管不調に関する情報は個人情報・要配慮個人情報として慎重に扱います。"
        f"賃管受験者は業務上必要な範囲で情報を共有し、病名の詳細を不必要に広げない対応が求められます。",
...
def _heading_フォローアップ(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"「復職したら終わり」ではなく、再発予防が賃管受験者の役割として問われます。",
...
def _heading_相談窓口(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"賃管受験者は相談を受けた場合の初動（傾聴、記録、担当者への連携）を理解しておく必要があります。",
...
def _heading_再発防止(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"再発防止には、ハラスメント防止方針の周知、相談窓口の明示、賃管受験者向け研修、"
...
def _heading_5ステップ(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"賃管受験者は③④⑤で中心役割を担いますが、医学的判断は主治医・産業医の領域です。"
...
def _heading_試験目的(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{EXAM}（賃管）は、賃管受験者が職場で賃管対策を実践するための"
        f"知識・態度を評価する試験です。部下の変化への気づき、安全配慮、賃管法令・制度、環境改善が中心テーマです。",
...
HEADING_MAP: dict[str, Callable[..., str]] = {
    ...
    "合格後の手続き": lambda t, s, g, c: two_paragraphs(
        ...
        f"{t}に合格した場合も、賃管受験者としての実務研修や社内規程の更新が別途必要なことがあります。"
    ...
    "賃管受験者の役割": _heading_賃管受験者役割,
    "賃管受験者の関与": _heading_賃管受験者役割,
...
def faq_answer_for(question: str, topic: str, slug: str, row: dict[str, str], faq_index: int = 1) -> str:
    ...
    # FAQ 役割分担で類似回答を避ける
    if faq_index == 1:
        if "賃管受験者" in q and ("診断" in q or "判断" in q):
            text = (
                "賃管受験者は医学的診断を行いません。不調の気づき、傾聴、安全配慮、"
        ...
        if "復職" in q or "復帰" in q:
            ...
            text = (
                f"試験では「{topic}」が、賃管受験者の行動として正しいか／主体を取り違えていないか、"
...
def lead_for(row: dict[str, str], topic: str) -> str:
    ...
    base = (
        f"{EXAM}の{topic}について、賃管受験者が現場で迷いやすい論点と"
...
def meta_description_for(row: dict[str, str], topic: str) -> str:
    ...
    if genre == "用語ハブ活用法":
        return (
            ...
            f"用語解説・演習・比較表との学習順と、賃管受験者が押さえる論点を解説します。"
        )[:165]
    if genre == "試験概要":
        return (
    ...
    return (
        f"{EXAM}の{topic}について、賃管受験者試験で問われる論点と学習の進め方を解説。"
