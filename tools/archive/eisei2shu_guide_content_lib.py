#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""第二種衛生管理者 試験ガイド向けの本文・FAQ 生成（量産テンプレ差し替え用）。"""
...
def _heading_独学前に公式情報を確認(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"独学を始める前に、{OFFICIAL}で受験資格・試験日程・出題範囲（3分野）・合格基準をメモしてください。"
...
def _heading_教材参考書(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{topic}向けには、{ORG}発行の公式テキスト（第二種衛生管理者）を軸に置くのが基本です。"
        f"問題集は年度・版の表記を確認し、3分野すべてをカバーしているかを目次でチェックしてください。",
...
def _heading_過去問現在地(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"最初の過去問は、{topic}に限定せず1回分を通しで解き、3分野のうちどこで迷ったかを記録します。"
        f"たとえば「労働衛生」と「法令」の問題を取り違えた、など分野名でメモすると復習が楽です。",
...
def _review_focus_label(topic: str, slug: str) -> str:
    if slug.startswith("exam-day-") or any(
        x in topic for x in ("持ち物", "流れ", "時間配分", "トラブル")
    ):
        return "演習で間違えた分野"
    return topic
...
def _heading_直前期絞り込み(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    ...
    return two_paragraphs(
        ...
        f"3分野のうち得点率が低い領域を1つ選び、用語10語＋演習20問程度をセットで見直すのが目安です。",
...
def _heading_分野位置づけ(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    ...
    return two_paragraphs(
        f"公式テキストの3分野のうち、「{field}」は{topic}の理解に直結する部分です。"
        f"衛生管理者試験では、現場の判断場面（気づき・相談・配慮・環境改善・専門職連携）として問われます。",
        f"他領域とのつながりを意識すると、事例問題の選択肢が読みやすくなります。"
        f"たとえば労働衛生の領域は、個別配慮や職場環境評価とセットで出題されることが多いです。",
    )
...
def _heading_他分野関連(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    ...
    return two_paragraphs(
        ...
        f"3分野をバランスよく学ぶため、弱点分野ばかりでなく、得意分野も月1回は演習で維持してください。"
...
def _heading_受験資格(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{EXAM}（第二種衛生管理者）の受験資格は、{OFFICIAL}の受験要項で確認します。"
...
def _heading_過去問年度別(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"過去問はまず1つの実施回を通しで解き、3分野ごとの正答率をざっくり把握します。"
...
def _heading_模試一問(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"一問一答は、通勤時間などの隙間に3分野を短時間で確認するのに向いています。"
...
def _heading_間違い分類(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"ノートの1行例：「第○問／労働衛生／『記録の開示範囲』を混同」。"
...
def _heading_試験目的(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{EXAM}（第二種衛生管理者）は、衛生管理者が職場で衛生管理対策を実践するための"
        f"知識・態度を評価する試験です。部下の変化への気づき、安全配慮、労働衛生、環境改善が中心テーマです。",
        f"3分野は公式テキストに沿って構成されています。"
...
def _heading_3分野(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    items = "\n".join(f"- {f}" for f in SEVEN_FIELDS)
    return two_paragraphs(
        f"出題範囲は公式テキストに沿った次の3分野です。\n{items}",
...
def _heading_試験会場基本情報(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
...
def _heading_試験日程確認(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{EXAM}の実施回・申込期間・試験日は、{OFFICIAL}の試験日程ページで公表されます。"
        f"{topic}での実施有無は、申込画面の受験地選択肢と要項PDFをセットで確認してください。",
        f"SNSやまとめサイトの日程は転載ミスがあり得るため、必ず公式サイトの更新日と照合してから"
        f"学習計画や有休取得を確定します。",
    )


def _heading_申込注意点(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"申込は受験要項の期間内に氏名・受験地・連絡先を正確に入力して完了させます。"
        f"{topic}で受験したい方は申込時点で該当地域を選び、受験料の支払期限をカレンダーに登録してください。",
        f"受験地は後から変更できない場合があるため、申込前にもう一度要項PDFの更新日を確認してください。"
        f"申込完了後に受験票や会場案内が届くまでの流れも要項で確認し、届かない場合の問い合わせ先を控えておきます。",
    )


def _heading_試験当日持ち物(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"受験票に記載された持ち物を前日のうちにカバンへ入れて準備してください。"
        f"一般的にはHB程度の黒い鉛筆と消しゴムが必要です。要項で身分証明書や受験票の持参が求められる場合があります。",
        f"筆記用具は予備を用意し、会場の禁止事項（スマートフォン・参考書・計算機など）を守ってください。"
        f"当日は早めに会場へ向かい、直前の詰め込みより睡眠と持ち物・会場確認を優先すると安心です。",
    )


def _heading_持込禁止(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"スマートフォン・タブレット・参考書・計算機・辞書などは、原則会場へ持ち込めません。"
        f"時計の持込可否も年度・会場により異なるため、{OFFICIAL}の受験要項と受験票で禁止物品リストを確認してください。",
        f"持込可能なのは要項で指定された筆記用具と受験票・身分証などに限られます。"
        f"荷物は会場の指示に従い、不要なものは試験外で保管する前提で向かってください。",
    )


def _heading_直前絞り込み見出し(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return _heading_直前期絞り込み(topic, _slug, _genre, _ctx)
...
def _heading_メンタルトラブル(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"問題が解けない場合も、全体の時間配分を崩さず次の設問へ進む判断が得点維持につながります。",
        f"会場でのトラブル（座席、マークシート、筆記用具など）は早めに係員へ声をかけてください。"
        f"{topic}の学習内容より、当日は冷静さと要項どおりの行動を優先します。",
    )


def _heading_公式情報確認先(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{topic}は、{OFFICIAL}（{ORG}）を最優先にしてください。"
        f"受験要項・試験日程・合格発表・テキスト改訂の案内は、非公式まとめより公式ページが正本です。",
        f"ブックマークしておき、学習中も月1回程度は更新日を確認する習慣をつけると、"
        f"古い数値や期限の誤認を防げます。",
    )


def _heading_サイトでできること(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return _heading_サイト学習(topic, _slug, _genre, _ctx)


def _heading_受験前押さえる(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return _heading_申込前チェック(topic, _slug, _genre, _ctx)


def _heading_次に読む記事(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{topic}を確認したら、試験ガイド一覧から自分の段階に合う記事へ進みます。"
        f"申込前は受験資格・日程記事、学習中は分野別ガイド、直前期は当日対策記事がおすすめです。",
        f"用語解説・演習問題・比較表は、ガイド記事で押さえた論点を具体問題で確認するために使います。"
        f"1テーマごとに演習5問と用語確認をセットにすると効率的です。",
    )


def _heading_まとめ(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{topic}での受験準備は、会場アクセスと申込・持ち物の確認が主なポイントです。"
        f"試験範囲の学習は公式テキストと演習問題で進め、日程・合格基準・会場情報だけは{OFFICIAL}で最新版を確認してください。",
        f"本サイトの用語解説・比較表・演習問題と併用すると、第二種衛生管理者試験全体の理解も効率的に深められます。",
    )
...
HEADING_MAP: dict[str, Callable[..., str]] = {
    ...
    "不合格後の立て直し": lambda t, s, g, c: two_paragraphs(
        ...
        f"演習の得点記録と誤答ノートを見返し、3分野のうち正答率が低い領域を2つに絞って1ヶ月集中する方法が有効です。",
    ...
    "公式情報の優先": lambda t, s, g, c: _heading_公式情報確認先(t, s, g, c),
    "まとめ": _heading_まとめ,
}
...
def _keyword_fallback(heading: str, topic: str, slug: str, genre: str) -> str:
    ...
    checks: list[tuple[tuple[str, ...], Callable[..., str]]] = [
        ...
        (("合格", "難易度", "合格率"), lambda t, s, g, c: two_paragraphs(
            ...
            f"{topic}の学習時間は、3分野のバランスと仕事の繁忙期を考慮して設定します。",
        )),
        (("直前", "試験前"), _heading_直前期絞り込み),
...
def faq_answer_for(question: str, topic: str, slug: str, row: dict[str, str], faq_index: int = 1) -> str:
    ...
    if not q:
        return ""
    if "持ち物" in q or "持参" in q or "忘れがち" in q:
        return ensure_min(
            "受験要項および受験票に記載された持ち物（HB程度の黒い鉛筆、消しゴム、必要なら身分証）を前日に準備してください。"
            "筆記用具は予備があると安心です。スマートフォン・参考書・計算機など禁止物品は要項どおり持ち込まないでください。",
            100,
            f"最新の持ち物リストは{ORG}の受験要項で確認してください。",
        )
    if "独学" in q and ("持ち物" in q or "当日" in topic or slug.startswith("exam-day-")):
        return ensure_min(
            "持ち物の準備は試験前日までに完了させ、当日は要項どおりの筆記用具と受験票・身分証を持参してください。"
            "学習面の独学対策とは別に、会場・開始時刻・禁止物品の確認を優先すると当日のトラブルを減らせます。",
            100,
            f"詳細は{OFFICIAL}の受験要項を確認してください。",
        )
    text = ""
    # FAQ 役割分担で類似回答を避ける
    if faq_index == 1:
...
def meta_description_for(row: dict[str, str], topic: str) -> str:
    ...
    return (
        f"{exam_topic_clause(EXAM, topic, EXAM_SHORT)}、衛生管理者試験で問われる論点と学習の進め方を解説。"
