...
"""知識ハブ CSV の内部バッチ表記（S37 等）と早見表重複を一括修正する。
  ...
  python3 tools/fix_hub_public_leaks.py --dry-run
  python3 tools/fix_hub_public_leaks.py --apply --rebuild
"""
...
from tools.archive.hub_dedup import load_csv, write_csv  # noqa: E402
from tools.archive.hub_matrix_repair import repair_hub_matrix_rows  # noqa: E402
from tools.archive.hub_strip_batch_suffix import strip_hub_rows  # noqa: E402

HUB_FILES = (
    ("comparisons.csv", "compare"),
    ("numbers.csv", "numbers"),
...
def fix_site(*, apply: bool, rebuild: bool, root: Path) -> dict[str, int]:
    ...
    for filename, _kind in HUB_FILES:
        path = data / filename
        if not path.is_file():
            continue
