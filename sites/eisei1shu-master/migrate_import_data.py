...
# -*- coding: utf-8 -*-
"""
一衛マスター（eisei1shu-master）の既存 HTML / CSV をテンプレ形式の data/*.csv に取り込む。

  python3 sites/eisei1shu-master/migrate_import_data.py --target /path/to/eisei1shu-master
  python3 sites/eisei1shu-master/migrate_import_data.py --target /path/to/eisei1shu-master --only glossary
"""
...
SUBJECT_TO_CATEGORY = {
    ...
    "労働衛生": "労働衛生（有害業務）",
    "労働生理": "労働生理",
    "労働衛生(有害業務に係るもの)": "労働衛生（有害業務）",
    "労働衛生（有害業務に係るもの）": "労働衛生（有害業務）",
    "関係法令(有害業務に係るもの)": "関係法令（有害業務）",
    "関係法令（有害業務に係るもの）": "関係法令（有害業務）",
}


def normalize_subject_category(subject: str) -> str:
    s = norm(subject).replace("(", "（").replace(")", "）")
    if s in SUBJECT_TO_CATEGORY:
        return SUBJECT_TO_CATEGORY[s]
    for key, cat in SUBJECT_TO_CATEGORY.items():
        if s.startswith(key[:4]):
            return cat
    return s

FIELD_NUM = {"関係法令": 1, "労働衛生": 2, "労働生理": 3}
...
def import_guide(target: Path) -> int:
    ...
    for path in sorted(articles_dir.glob("*.html")):
        ...
        category = extract_block(html, r'<span\s+class="meta-category"[^>]*>(.*?)</span>')
        genre = CATEGORY_TO_GENRE.get(category, "試験概要")
        updated = ""
        ...
        if body and not lead_parts:
            pm = re.search(r"<p>(.*?)</p>", body, re.I | re.S)
            if pm:
                lead_parts.append(strip_tags(pm.group(1)))
        lead = "\n\n".join(lead_parts[:2]) or (sections[0][1][:300] if sections else title)
        ...
        row: dict[str, str] = {
            ...
            "tags": "第一種衛生管理者;試験ガイド",
            "author_name": "一衛マスター編集部",
            "author_profile": "第一種衛生管理者試験の学習コンテンツを整理する編集チーム",
            "reviewer_name": "一衛マスター確認担当",
            "reviewer_profile": "公開前に公式情報と内部リンクを確認",
            "fact_checked_at": updated or "2026-05-18",
            ...
            "last_reviewed_at": updated or "2026-05-18",
            "next_review_at": "2026-08-18",
            "source_checked_at": updated or "2026-05-18",
            "content_status": "published",
            "revision_note": "migrate_import_data.py で移行",
            "faq_1_question": f"{title}はどこで公式情報を確認しますか？",
...
def parse_era_year(開催年数: str) -> tuple[int, str]:
    raw = norm(開催年数)
    for pat, base in ((r"令和(\d+)年", 2018), (r"平成(\d+)年", 1988), (r"昭和(\d+)年", 1925)):
        m = re.match(pat, raw)
        if m:
...
def import_ichimon(target: Path) -> int:
    src = target / "docs" / "eisei1_ichimon_itto.csv"
    if not src.is_file():
        print(f"skip ichimon: {src} not found")
        return 0
    out_rows: list[dict[str, str]] = []
    with src.open(encoding="utf-8-sig", newline="") as f:
        for row in csv.DictReader(f):
            subject = norm(row.get("科目"))
            cat = normalize_subject_category(subject)
            ans = norm(row.get("答え"))
            if ans not in ("○", "〇", "×", "✕", "╳"):
                continue
            out_rows.append(
                {
                    "id": norm(row.get("id")),
                    "question": norm(row.get("問題")),
                    "answer": ans,
                    "explanation": norm(row.get("解説")),
                    "category": cat,
                    "tags": norm(row.get("タグ")) or subject,
                    "source": norm(row.get("開催年数")),
                    "note": "",
                }
            )
    out_path = target / "data" / "ichimon_questions.csv"
    header = ["id", "question", "answer", "explanation", "category", "tags", "source", "note"]
    with out_path.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=header, lineterminator="\n")
        w.writeheader()
        w.writerows(out_rows)
    print(f"ichimon_questions.csv: {len(out_rows)} rows -> {out_path}")
    return len(out_rows)


def import_past(target: Path) -> int:
    src = target / "data" / "eisei1_past_questions.csv"
    if not src.is_file():
        print(f"skip past: {src} not found")
        return 0
    with src.open(encoding="utf-8-sig", newline="") as f:
    ...
    for row in src_rows:
        key = (norm(row.get("開催年数")), norm(row.get("開催月")))
        pool.setdefault(key, []).append(row)

    out_rows: list[dict[str, str]] = []
    pool_year_base = 1990
    for pool_idx, ((_era, _month), group) in enumerate(sorted(pool.items())):
        exam_year = pool_year_base + pool_idx
        wareki = f"{_era}{_month}月" if _month else _era
        qno = 0
        def _subject_order(r: dict[str, str]) -> int:
            sub = norm(r.get("科目"))
            for key, num in FIELD_NUM.items():
        ...
        for row in sorted(
            group,
            key=lambda r: (_subject_order(r), int(norm(r.get("問番号")) or "0")),
        ):
            qno += 1
            subject = norm(row.get("科目"))
            cat = normalize_subject_category(subject)
            stem = norm(row.get("問"))
            expl = norm(row.get("解説"))
            correct = norm(row.get("正答番号"))
            choices = [norm(row.get(f"({i})")) for i in range(1, 6)]
            out_rows.append(
                {
                    ...
                    "note": "",
                    "explanation": expl,
                    "explanation_summary": expl[:200] if expl else "",
                    "explanation_correct": "",
                    "explanation_choices": "",
                    "explanation_point": "関連用語と同年の他の過去問で定着を確認してください。",
                    "related_links": "",
                }
            )

    out_path = target / "data" / "past_questions.csv"
    with out_path.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=PAST_HEADER, lineterminator="\n")
        w.writeheader()
        w.writerows(out_rows)
    print(f"past_questions.csv: {len(out_rows)} rows -> {out_path}")
    enrich = target / "tools" / "enrich_past_explanation_choices.py"
    if enrich.is_file():
        import subprocess

        subprocess.run(
            [sys.executable, str(enrich), "--csv", str(out_path)],
            cwd=target,
            check=True,
        )
    return len(out_rows)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="一衛マスター data CSV 移行")
    ap.add_argument("--target", required=True, type=Path)
    ap.add_argument(
        "--only",
        choices=("all", "glossary", "guide", "past", "ichimon", "config"),
        default="all",
    )
    args = ap.parse_args()
    ...
    if only in ("all", "past"):
        import_past(target)
    if only in ("all", "ichimon"):
        import_ichimon(target)

    if only in ("all", "practice"):
        stub = SITE_DIR / "practice_questions.stub.csv"
        dst = target / "data" / "practice_questions.csv"
        if stub.is_file():
            dst.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(stub, dst)
            print(f"practice_questions.csv: stub -> {dst}")
