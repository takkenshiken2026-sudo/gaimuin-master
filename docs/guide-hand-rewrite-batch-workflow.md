...
**編集合格（お手本水準）の全件プログラム:** [guide-expert-rewrite-program.md](./guide-expert-rewrite-program.md)  
**お手本:** 基本 `exam-schedule` / `tools/mankan_rewrite_exemplar.py`、具体例 `study-plan-6months` / `tools/mankan_rewrite_exemplar_concrete.py`（マン管サイト）
...
| `revision_note` | `手書きリライト・具体例` を含める（§3.1.1 参照） |
...
# 2) 適用 → パディング除去 → **当該5 slug のみ** CSV/監査（enrich なし）
python3 tools/run_guide_hand_batch.py --batch tools/mankan_rewrite_batch19.py
# ※ 他記事の180字未満は batch 完了まで ERROR にしない（--only-slugs で限定検証）
